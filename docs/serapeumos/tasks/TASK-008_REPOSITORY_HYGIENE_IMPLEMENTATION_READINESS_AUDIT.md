...
perform it only in an isolated disposable workspace beneath the canonical root.

Disposable audit workspace:

D:\SerapeumOS\temp\TASK-008_HYGIENE_AUDIT

The resolved workspace path must be verified as beneath D:\SerapeumOS before
creation. The root `.gitignore` already ignores `temp/*`, so no canonical
untracked content need be copied into it.

The isolated workspace must contain only tracked repository content from
TASK-008_EXECUTION_BASELINE plus artifacts generated by the audit itself.

Do not copy canonical untracked content into it.

Preferred construction:
create a tracked-only disposable clone/worktree beneath the ignored
`temp/` directory.

Whichever mechanism is used:
- prove its resolved path is beneath D:\SerapeumOS
- prove its checked-out SHA equals TASK-008_EXECUTION_BASELINE
- prove canonical tracked content remains unchanged
- _archsync_input/ must remain untouched and uninspected.

Audit workspace checkout SHA must equal TASK-008_EXECUTION_BASELINE.
